...
def evaluate_dmv_entry(entry):
    ...
    prompt = f"""
        ...
        Evaluate whether the quoted DMV content clearly describes the retention or transfer of STANDARD license plates during a 
        vehicle sale or vehicle ownership. The policy can fall into one of three categories: retained by the seller, 
            should be transferred with the vehicle, or surrendered to the DMV when a vehicle is sold or transferred. 

        Your task is to classify the quote and provide a summary of the policy, as well as a confidence level based on the clarity and relevance of the quote.
        ...
        - OR The content describes that plates can be TRANSFERRED to a different vehicle with the same owner, or registration can be transferred, even if a sale is not explicitly mentioned
